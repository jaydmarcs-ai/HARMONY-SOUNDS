...
function Team({ profiles, currentProfile }) {
  ...
  const save = async (u) => {
    ...
    setSending(true);
    const { data, error, otpError } = await profilesApi.invite(u);
    setSending(false);
    if (error) { alert("Couldn't create the invite: " + error.message); return; }
    setEditing(null);
    setInvited({ ...data, otpError });
  };
  const remove = async (id) => { await profilesApi.remove(id); setEditing(null); };
  const resend = async (email) => { await profilesApi.resend(email); };

  return (
    <div>
      <div className="hs-toolbar"><span className="hs-muted">{profiles.length} team member(s)</span><button className="hs-btn primary" onClick={() => setEditing({ name: "", email: "", role: "staff" })}><Plus size={16} /> Invite Team Member</button></div>
      <p className="hs-muted" style={{ marginTop: -10, marginBottom: 16 }}>Invites send a real sign-in email through Supabase. They click it and land on their role's dashboard automatically.</p>
      <div className="hs-tag-grid">
        {profiles.map((u) => {
            ...
            <div key={u.id} className="hs-client-card">
              ...
              {u.status === "invited" && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6 }}>
                  <span className="hs-badge warn">Invite pending</span>
                  <button className="hs-btn small" onClick={(e) => { e.stopPropagation(); resend(u.email); }}><Mail size={12} /> Resend</button>
                </div>
              )}
            </div>
          );
      ...
      {editing && (
        <Modal title={editing.id ? "Edit Person" : "Invite Team Member"} onClose={() => setEditing(null)}>
          <Field label="Name"><input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} /></Field>
          ...
          <div className="hs-modal-actions">
            ...
            <button className="hs-btn primary" onClick={() => save(editing)} disabled={!editing.name || (!editing.id && !editing.email) || sending}>{sending ? "Sending..." : editing.id ? "Save" : "Send Invite"}</button>
          </div>
        </Modal>
      )}
      {invited && (
        <Modal title="Invite Sent" onClose={() => setInvited(null)}>
          <p className="hs-muted">A real sign-in email was sent to <strong>{invited.email}</strong>. Once they click the link, they'll land on their {ROLES.find((r) => r.key === invited.role)?.label} dashboard.</p>
          {invited.otpError && <p className="hs-danger-text" style={{ marginTop: 8 }}>Note: the email may not have sent — {invited.otpError.message}</p>}
          <div className="hs-modal-actions"><button className="hs-btn primary" onClick={() => setInvited(null)}>Got it</button></div>
        </Modal>
      )}
    </div>
  );
