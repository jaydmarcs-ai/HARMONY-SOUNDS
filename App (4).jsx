...
function Team({ profiles, currentProfile }) {
  ...
  const [resetTarget, setResetTarget] = useState(null); // {name, email}
  ...
  const openReset = (u) => { setResetTarget(u); setNewPassword(""); setResetError(""); setResetDone(false); setEditing(null); };
  const submitReset = async () => {
    if (newPassword.length < 6) { setResetError("Password must be at least 6 characters."); return; }
    setResetting(true); setResetError("");
    const { error } = await profilesApi.adminResetPassword(resetTarget.email, newPassword);
    setResetting(false);
    if (error) setResetError(error.message); else setResetDone(true);
  };
    ...
    <div>
      ...
      {editing && (
        <Modal title={editing.id ? "Edit Person" : "Pre-set a Role"} onClose={() => setEditing(null)}>
          ...
          <div className="hs-modal-actions">
            {editing.id && editing.id !== currentProfile.id && editing.status !== "invited" && <button className="hs-btn" onClick={() => openReset(editing)}>Reset Password</button>}
      ...
      {resetTarget && (
        <Modal title={`Reset Password — ${resetTarget.name}`} onClose={() => setResetTarget(null)}>
          {resetDone ? (
            <>
              <p className="hs-muted">Password updated for {resetTarget.name}. Share the new password with them directly.</p>
              <div className="hs-modal-actions"><button className="hs-btn primary" onClick={() => setResetTarget(null)}>Done</button></div>
            </>
          ) : (
            <>
              ...
              {resetError && <p className="hs-danger-text">{resetError}</p>}
              <div className="hs-modal-actions">
                <button className="hs-btn" onClick={() => setResetTarget(null)}>Cancel</button>
                <button className="hs-btn primary" onClick={submitReset} disabled={!newPassword || resetting}>{resetting ? "Resetting..." : "Reset Password"}</button>
              </div>
            </>
          )}
        </Modal>
...
function ChangePasswordModal({ onClose }) {
  ...
  return (
    <Modal title="Change Password" onClose={onClose}>
      {done ? (
        <>
          <p className="hs-muted">Your password has been updated.</p>
          <div className="hs-modal-actions"><button className="hs-btn primary" onClick={onClose}>Done</button></div>
        </>
      ) : (
        <>
          <Field label="New Password"><input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="At least 6 characters" /></Field>
          ...
          {error && <p className="hs-danger-text">{error}</p>}
          <div className="hs-modal-actions">
            <button className="hs-btn" onClick={onClose}>Cancel</button>
            <button className="hs-btn primary" onClick={submit} disabled={!newPassword || !confirm || saving}>{saving ? "Saving..." : "Update Password"}</button>
          </div>
        </>
      )}
    </Modal>
        ...
        <>
          <div className="hs-sidebar">
            ...
            <div className="hs-sidebar-foot">
              ...
              <button className="hs-switch-btn" onClick={() => setChangingPassword(true)} style={{ marginBottom: 6 }}>Change Password</button>
